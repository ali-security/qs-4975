...
test('combine()', function (t) {
    t.test('combines into a single array', function (st) {
        var combined = utils.combine('a', 'b', 10, false);
        st.deepEqual(combined, ['a', 'b'], 'two strings combined into an array');
        st.end();
    });

    t.test('combines multiple values', function (st) {
        var combined = utils.combine(['a', 'b'], 'c', 10, false);
        st.deepEqual(combined, ['a', 'b', 'c'], 'array and string combined');
        st.end();
    });
